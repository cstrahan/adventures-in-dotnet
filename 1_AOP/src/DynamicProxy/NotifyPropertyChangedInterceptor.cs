        ...
        public void Intercept(IInvocation invocation)
        {
            ...
            if (invocation.Method.Name.StartsWith("set_"))
            {
                ...
                if (info != null)
                {
                    ...
                    PropertyChangedEventHandler handler = info.GetValue(invocation.InvocationTarget) as PropertyChangedEventHandler;
                    // invoke the delegate if it's not null (aka empty)
                    if (handler != null)
                        handler.Invoke(invocation.TargetType, new PropertyChangedEventArgs(propertyName));
                }
            }
        }
    }
